  ...
  <div class="front-container" style="width: 80%">
    <div style="margin-bottom: 10px">
      <el-input
        ...
        style="width: 400px; height: 40px; margin-right: 10px"
    ...
    <div class="card">
      <el-table
        :data="data.tableData"
        stripe
        :cell-style="{ backgroundColor: '#e8efff' }"
        default-expand-all
      >
            ...
            <div style="padding: 10px">
              <el-table :data="props.row.orderDetailList" border>
                ...
                <el-table-column label="小计" width="150">
                  <template #default="scope">
                    <b style="color: red"
                      >{{ (scope.row.goodsPrice * scope.row.num).toFixed(2) }} 元</b
                    >
                  </template>
                </el-table-column>
        ...
        <el-table-column label="订单操作" align="center" width="120">
          <template #default="scope">
            <el-button @click="cancel(scope.row)" type="danger" v-if="scope.row.status === '待接单'"
              >取消</el-button
            >
            <el-button
...
const cancel = (row) => {
  ElMessageBox.confirm('您确认取消订单吗?', '二次确认', { type: 'warning' })
    .then((res) => {
      data.form = row
...
const done = (row) => {
  ElMessageBox.confirm('您确认订单货物已经收到了吗?', '二次确认', { type: 'warning' })
    .then((res) => {
      data.form = row
  ...
  request
    ...
    .then((res) => {
      ...
      data.form =
        res.data?.length > 0
          ? res.data[0]
          : { orderId: row.orderId, goodsId: row.goodsId, userId: data.user.id }
