  ...
  <div class="front-container" style="width: 50%">
      ...
      <div style="flex: 1">
        <div style="display: flex; align-items: flex-start; grid-gap: 20px; margin-bottom: 10px">
          <div style="font-size: 22px; font-weight: bold; line-height: 25px; flex: 1">
            <el-tag
              style="margin-right: 5px; float: left; background-color: red; color: white"
              type="danger"
              v-if="data.goods.recommend === '是'"
              >推荐</el-tag
            >
            {{ data.goods.name }}
          </div>
          <div
            style="width: 60px; cursor: pointer; color: #666"
            @click="addCollect"
            v-if="!data.userCollect?.id"
          >
            <el-icon style="position: relative; top: 3px" size="18"><Star /></el-icon>收藏
          </div>
          <div
            style="width: 100px; cursor: pointer; color: orange"
            @click="removeCollect"
            v-if="data.userCollect?.id"
          >
        ...
        <div style="margin-bottom: 20px">
          ...
          ><b style="color: red; font-size: 30px">{{ data.goods.price }}</b>
        ...
        <div>
          <el-input-number
            ...
            v-model="data.num"
          ></el-input-number>
          <el-button @click="addCart" style="height: 40px; margin-left: 5px" type="danger"
            >加入购物车</el-button
          >
          <el-button @click="handleAddOrder" style="height: 40px; margin-left: 5px" type="danger"
            >立即购买</el-button
          >
        </div>
        <div style="margin-top: 10px; color: #666">
          店铺销售并发货的商品，由小卖部提供发票和相应的售后服务。请您放心购买！
        </div>
      </div>
    </div>
    ...
    <div class="card" style="padding: 20px; margin-bottom: 50px">
      <div style="font-size: 20px; padding-bottom: 10px; border-bottom: 1px solid #ddd">
        <span
          @click="changeTab('商品详情')"
          style="cursor: pointer"
          :class="{ 'current-active': data.current === '商品详情' }"
          >商品详情</span
        >
        <span
          @click="changeTab('商品评价')"
          :class="{ 'current-active': data.current === '商品评价' }"
          style="cursor: pointer; margin-left: 20px"
          >商品评价</span
        >
      </div>
      <div
        v-if="data.current === '商品详情'"
        style="padding: 10px"
        v-html="data.goods.content"
      ></div>
      <div v-if="data.current === '商品评价'" style="min-height: 700px">
        ...
        <div v-if="data.commentList.length > 0" style="padding: 20px">
          ...
          <div
            ...
            :key="item.id"
            style="display: flex; grid-gap: 10px; padding: 10px 0; border-bottom: 1px solid #ddd"
...
const addCart = () => {
  request
    .post('/cart/add', { goodsId: data.id, num: data.num, userId: data.user.id })
    .then((res) => {
      if (res.code === '200') {
