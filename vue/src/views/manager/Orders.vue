    ...
    <div class="card" style="margin-bottom: 5px">
      ...
      >
        ...
        <el-table-column label="订单操作" align="center" width="100">
          <template #default="scope">
            <el-button
              v-if="scope.row.deliverType === '自提' && scope.row.status === '待接单'"
              type="primary"
              @click="out(scope.row)"
              >出货</el-button
            >
            <el-button
              v-if="scope.row.deliverType === '外送' && scope.row.status === '待接单'"
              type="primary"
...
const out = (row) => {
  ElMessageBox.confirm('您确认订单已出货吗?', '二次确认', { type: 'warning' })
    .then((res) => {
      data.form = row
